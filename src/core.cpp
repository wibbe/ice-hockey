...
void CORE_PlayLoopSound (unsigned loopchannel, ALuint snd, float volume, float pitch)
{
  if (loopchannel >= SND_MAX_LOOP_SOUNDS)
    return;
  alSourceStop(SND_Sources[SND_MAX_SOURCES-loopchannel-1]);
  alSourcei(SND_Sources[SND_MAX_SOURCES-loopchannel-1], AL_BUFFER, snd);
  alSourcef(SND_Sources[SND_MAX_SOURCES-loopchannel-1], AL_GAIN,   volume);
  alSourcei(SND_Sources[SND_MAX_SOURCES-loopchannel-1], AL_LOOPING, AL_TRUE);
  alSourcePlay(SND_Sources[SND_MAX_SOURCES-loopchannel-1]);
}
...
void CORE_SetLoopSoundParam (unsigned loopchannel, float volume, float pitch)
{
  if (loopchannel >= SND_MAX_LOOP_SOUNDS)
    return;
  alSourcef(SND_Sources[SND_MAX_SOURCES-loopchannel-1], AL_GAIN,   volume);
  alSourcef(SND_Sources[SND_MAX_SOURCES-loopchannel-1], AL_PITCH,  pitch);
}
...
//-----------------------------------------------------------------------------
#define MAX_WAV_SIZE 40*1024*1024 // Max 40Mb sound! (for music)
